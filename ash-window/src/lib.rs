...
/// enabled, acquired through [`enumerate_required_extensions()`].
///
/// # Safety
///
/// There is a [parent/child relation] between [`Instance`] and [`Entry`], and
/// the resulting [`vk::SurfaceKHR`].  The application must not
/// [destroy][Instance::destroy_instance()] these parent objects before first
/// [destroying][surface::Instance::destroy_surface()] the returned
/// [`vk::SurfaceKHR`] child object.  [`vk::SurfaceKHR`] does _not_ implement
/// [drop][drop()] semantics and can only be destroyed via
/// [`destroy_surface()`][surface::Instance::destroy_surface()].
///
/// See the [`Entry::create_instance()`] documentation for more destruction
/// ordering rules on [`Instance`].
///
/// The window represented by `window_handle` must be associated with the
/// display connection in `display_handle`.
///
/// `window_handle` and `display_handle` must be associated with a valid window
/// and display connection, which must not be destroyed for the lifetime of the
/// returned [`vk::SurfaceKHR`].
///
...
/// Query the required instance extensions for creating a surface from a raw
/// display handle.
///
/// This [`RawDisplayHandle`] can typically be acquired from a window, but is
/// usually also accessible earlier through an "event loop" concept to allow
/// querying required instance extensions and creation of a compatible Vulkan
/// instance prior to creating a window.
///
