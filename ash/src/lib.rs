...
//! ## Getting started
//!
//! Load the Vulkan library linked at compile time using [`Entry::linked()`], or
//! load it at runtime using [`Entry::load()`], which uses `libloading`. If you
//! want to perform entry point loading yourself, call
//! [`Entry::from_static_fn()`].
//!
//! ## Crate features
//!
//! * **debug** (default): Whether Vulkan structs should implement `Debug`.
//! * **loaded** (default): Support searching for the Vulkan loader manually at
//!   runtime.
...
pub use crate::{
    device::Device, entry::Entry, extensions_generated::*, instance::Instance, tables::*,
};
...
/// Given a mutable raw pointer to a type with an `s_type` member such as
/// [`vk::BaseOutStructure`], match on a set of Vulkan structures. The struct
/// will be rebound to the given variable of the type of the given Vulkan
/// structure.
///
/// Note that all match bodies have to be enclosed by curly braces due to macro
/// parsing limitations. It is unfortunately not possible to write `x @
/// ash::vk::SomeStruct => one_line_expression(),`.
///
/// ```
...
/// ```
///
/// In addition this macro propagates implicit return values just like normal
/// `match` blocks, as long as a default value or expression is provided in the
/// "any" match arm (`_ => { some_value() }`). For the time being said arm must
/// be wrapped in curly braces; an expression like `_ => None` is not yet
/// supported.
///
/// ```
/// # let mut info = ash::vk::DeviceCreateInfo::default();
...
/// Given an immutable raw pointer to a type with an `s_type` member such as
/// [`vk::BaseInStructure`], match on a set of Vulkan structures. The struct
/// will be rebound to the given variable of the type of the given Vulkan
/// structure.
///
/// Note that all match bodies have to be enclosed by curly braces due to macro
...
/// ```
///
/// See the [`match_out_struct!`] documentation for an example with implicit
/// return values.
#[macro_export]
